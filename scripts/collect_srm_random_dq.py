...
argparser.add_argument('-u', dest='voltage', type=float, default=48, help='电压控制量绝对值的最大值')
argparser.add_argument('-i', dest='current', type=float, default=10, help='状态量电流绝对值的最大值')
argparser.add_argument('-v', dest='vel', type=float, default=1e3, help='状态量速度绝对值的最大值')
...
argparser.add_argument('-low', dest='u_low', default=0, type=float, help='电压控制低值')
argparser.add_argument('-high', dest='u_high', default=20, type=float, help='电压控制高值')
...
if __name__ == '__main__':
    ...
    filename = '_rand_Udq=' + f'{args.u_low}V-{args.u_high}V' + filename
    ...
    with open(filename, 'w', newline='') as csvfile:
        ...
        for _ in tqdm.trange(int(args.num_samples), desc='Collecting:'):
            if done:
                ...
                env.reset(only_step=True, random_state=False)  # 重置step
                num_episodes += 1
